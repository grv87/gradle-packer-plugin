...
class PackerPluginExtension /*extends PackerToolExtension*/ {
  ...
  Closure configureClosure(Directory templateWorkingDir, @DelegatesTo(PackerVarArgument) Closure taskConfiguration) { // TODO
    { PackerVarArgument packerWrapperTask ->
      packerWrapperTask.environment environment
      packerWrapperTask.workingDir.set templateWorkingDir
      packerWrapperTask.variables = variables
      packerWrapperTask.configure taskConfiguration
    }
  }

  Object template(String name, File file, boolean oneTaskPerBuild = true, @DelegatesTo(PackerBuildAutoConfigurable) Closure taskConfiguration = null) {
    project.logger.debug(sprintf('org.fidata.packer: Processing %s template', [file]))

    Directory templateWorkingDir = workingDir.getOrElse(project.layout.projectDirectory.dir(file.parentFile.absoluteFile.toString()))

    Template template = Template.readFromFile(file)
    template.interpolate new Context(stringize(variables), stringize(environment), file, templateWorkingDir.asFile.toPath(), project)
    ...
    TaskProvider<PackerValidate> validateProvider = project.tasks.register("$PackerBasePlugin.PACKER_VALIDATE_TASK_NAME-$name".toString(), PackerValidate) { PackerValidate validate ->
      ...
      validate.configure configureClosure(templateWorkingDir, taskConfiguration)
    }
    ...
    project.tasks.register("packerBuild-$aName".toString(), PackerBuildAutoConfigurable, file, template.clone(), new OnlyExcept(), configureClosure(templateWorkingDir, taskConfiguration))
    // }
  }

  Object template(File file, boolean oneTaskPerBuild = true, @DelegatesTo(PackerVarArgument) Closure taskConfiguration = null) {
    template(null, file, oneTaskPerBuild, taskConfiguration)
  }

  Object template(Map<String, File> files, boolean oneTaskPerBuild = true, @DelegatesTo(PackerVarArgument) Closure taskConfiguration = null) {
  ...
  Object template(List<File> files, boolean oneTaskPerBuild = true, @DelegatesTo(PackerVarArgument) Closure taskConfiguration = null) {
