...
class ContextTestResolveUriWindowsFiles {
  ...
  public static final File testFixturesDir = temporaryFolder.newFolder('test-fixtures')
  ...
  private static final String NATIVE_PATH_TO_TEST_FIXTURES = testFixturesDir.absoluteFile.toString()
  ...
  void test(final String original, final String expected) {
    assert new Context(null, null, null, temporaryFolder.root.toPath(), null).resolveUri(original).toString() == expected
  }
}
